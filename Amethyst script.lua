-- ESP + Skeleton + Aimbot (Head/Torso) + Triggerbot (Alt) + Fly (C) + NoClip + Вкладки Visual/Main (сворачиваются)
-- Меню: перетаскивается, Правый Ctrl – свернуть/развернуть
-- Все элементы меню двигаются вместе с вкладками, в подсказке указано управление
...
menuFrame.Size = UDim2.new(0, 300, 0, 440)
menuFrame.Position = UDim2.new(1, -310, 0.5, -220)
menuFrame.BackgroundColor3 = Color3.fromRGB(100, 60, 140)
menuFrame.BackgroundTransparency = 0.3
menuFrame.Active = true
menuFrame.Draggable = true
menuFrame.BorderSizePixel = 1
menuFrame.BorderColor3 = Color3.fromRGB(180, 130, 220)
menuFrame.ZIndex = 10
pcall(function()
...
title.BackgroundColor3 = Color3.fromRGB(70, 30, 100)
title.Text = "Меню"
...
-- Вкладки
local tabVisual = Instance.new("TextButton", menuFrame)
tabVisual.Size = UDim2.new(0, 140, 0, 28)
tabVisual.Position = UDim2.new(0, 8, 0, 35)
tabVisual.BackgroundColor3 = Color3.fromRGB(130, 90, 170)
tabVisual.TextColor3 = Color3.new(1,1,1)
tabVisual.Font = Enum.Font.SourceSansBold
tabVisual.TextSize = 13
tabVisual.Text = "▼ Visual"
tabVisual.ZIndex = 11
pcall(function() local c = Instance.new("UICorner", tabVisual); c.CornerRadius = UDim.new(0, 5) end)

local tabMain = Instance.new("TextButton", menuFrame)
tabMain.Size = UDim2.new(0, 140, 0, 28)
tabMain.Position = UDim2.new(0, 152, 0, 35)
tabMain.BackgroundColor3 = Color3.fromRGB(90, 70, 130)
tabMain.TextColor3 = Color3.new(1,1,1)
tabMain.Font = Enum.Font.SourceSansBold
tabMain.TextSize = 13
tabMain.Text = "▼ Main"
tabMain.ZIndex = 11
pcall(function() local c = Instance.new("UICorner", tabMain); c.CornerRadius = UDim.new(0, 5) end)

-- Фреймы содержимого
local visualFrame = Instance.new("Frame", menuFrame)
visualFrame.Size = UDim2.new(1, -16, 0, 120)
visualFrame.Position = UDim2.new(0, 8, 0, 68)
visualFrame.BackgroundTransparency = 1
visualFrame.ZIndex = 11
visualFrame.Visible = true

local mainFrame = Instance.new("Frame", menuFrame)
mainFrame.Size = UDim2.new(1, -16, 0, 250)
mainFrame.Position = UDim2.new(0, 8, 0, 68)
mainFrame.BackgroundTransparency = 1
mainFrame.ZIndex = 11
mainFrame.Visible = false

local visualExpanded = true
local mainExpanded = true

-- Динамические кнопки
local flyButton, noClipButton, hintLabel

local function updateMenuHeight()
    local yOffset = 68
    if visualExpanded and visualFrame.Visible then
        yOffset = yOffset + 120 + 5
    end
    if mainExpanded and mainFrame.Visible then
        yOffset = yOffset + 250 + 5
    end
    if flyButton then
        flyButton.Position = UDim2.new(0, 8, 0, yOffset)
    end
    if noClipButton then
        noClipButton.Position = UDim2.new(0, 8, 0, yOffset + 34)
    end
    if hintLabel then
        hintLabel.Position = UDim2.new(0, 8, 0, yOffset + 70)
    end
    menuFrame.Size = UDim2.new(0, 300, 0, yOffset + 115)
end

-- Создаём Fly и NoClip кнопки
flyButton = Instance.new("TextButton", menuFrame)
flyButton.Size = UDim2.new(1, -16, 0, 30)
flyButton.BackgroundColor3 = FLY_ENABLED and Color3.fromRGB(60, 140, 60) or Color3.fromRGB(140, 60, 60)
flyButton.TextColor3 = Color3.new(1,1,1)
flyButton.Font = Enum.Font.SourceSansBold
flyButton.TextSize = 13
flyButton.Text = "Fly: "..(FLY_ENABLED and "ВКЛ" or "ВЫКЛ")
flyButton.AutoButtonColor = false
flyButton.ZIndex = 11
pcall(function() local c = Instance.new("UICorner", flyButton); c.CornerRadius = UDim.new(0, 5) end)
...
noClipButton = Instance.new("TextButton", menuFrame)
noClipButton.Size = UDim2.new(1, -16, 0, 30)
noClipButton.BackgroundColor3 = NOCLIP_ENABLED and Color3.fromRGB(60, 140, 60) or Color3.fromRGB(140, 60, 60)
noClipButton.TextColor3 = Color3.new(1,1,1)
noClipButton.Font = Enum.Font.SourceSansBold
noClipButton.TextSize = 13
noClipButton.Text = "NoClip: "..(NOCLIP_ENABLED and "ВКЛ" or "ВЫКЛ")
noClipButton.AutoButtonColor = false
noClipButton.ZIndex = 11
pcall(function() local c = Instance.new("UICorner", noClipButton); c.CornerRadius = UDim.new(0, 5) end)
...
hintLabel = Instance.new("TextLabel", menuFrame)
hintLabel.Size = UDim2.new(1, -16, 0, 40)
hintLabel.BackgroundTransparency = 1
hintLabel.TextColor3 = Color3.fromRGB(220, 200, 255)
hintLabel.Font = Enum.Font.SourceSans
hintLabel.TextSize = 11
hintLabel.Text = "Aimbot: Alt | Fly: C\nПравый Ctrl – свернуть меню"
hintLabel.ZIndex = 11
hintLabel.TextWrapped = true

-- Переключение вкладок
local function toggleVisual()
    visualExpanded = not visualExpanded
    visualFrame.Visible = visualExpanded
    tabVisual.Text = visualExpanded and "▼ Visual" or "▶ Visual"
    updateMenuHeight()
end

local function toggleMain()
    mainExpanded = not mainExpanded
    mainFrame.Visible = mainExpanded
    tabMain.Text = mainExpanded and "▼ Main" or "▶ Main"
    updateMenuHeight()
end
...
tabMain.MouseButton1Click:Connect(function()
    if mainFrame.Visible then
        ...
        tabVisual.Text = "▶ Visual"
        tabMain.Text = "▼ Main"
        tabVisual.BackgroundColor3 = Color3.fromRGB(90, 70, 130)
        tabMain.BackgroundColor3 = Color3.fromRGB(130, 90, 170)
        updateMenuHeight()
    end
end)

-- Функция для кнопок внутри фреймов
local function makeButtonInFrame(parent, y, name, state, cb)
    ...
    btn.ZIndex = 11
    pcall(function() local c = Instance.new("UICorner", btn); c.CornerRadius = UDim.new(0, 5) end)
    local active = state
...
makeButtonInFrame(visualFrame, 0, "ESP", ESP_ENABLED, function(v) ESP_ENABLED = v end)
makeButtonInFrame(visualFrame, 34, "Skeleton", SKELETON_ENABLED, function(v) SKELETON_ENABLED = v end)
makeButtonInFrame(visualFrame, 68, "Allies ESP", SHOW_ALLIES, function(v) SHOW_ALLIES = v end)

makeButtonInFrame(mainFrame, 0, "Aimbot", AIMBOT_ENABLED, function(v) AIMBOT_ENABLED = v end)
makeButtonInFrame(mainFrame, 34, "Triggerbot", TRIGGERBOT_ENABLED, function(v) TRIGGERBOT_ENABLED = v end)

-- Aim Part
local aimPartBtn = Instance.new("TextButton", mainFrame)
aimPartBtn.Size = UDim2.new(1, 0, 0, 30)
aimPartBtn.Position = UDim2.new(0, 0, 0, 68)
aimPartBtn.BackgroundColor3 = Color3.fromRGB(60, 100, 140)
...
pcall(function() local c = Instance.new("UICorner", aimPartBtn); c.CornerRadius = UDim.new(0, 5) end)
...
fovSlider.Position = UDim2.new(0, 40, 0, 110)
fovSlider.BackgroundColor3 = Color3.fromRGB(80, 80, 90)
fovSlider.BorderSizePixel = 0
fovSlider.ZIndex = 11
pcall(function() local c = Instance.new("UICorner", fovSlider); c.CornerRadius = UDim.new(0, 4) end)
...
fovFill.BackgroundColor3 = Color3.fromRGB(180, 140, 255)
fovFill.BorderSizePixel = 0
fovFill.ZIndex = 12
pcall(function() local c = Instance.new("UICorner", fovFill); c.CornerRadius = UDim.new(0, 4) end)

local fovInput = Instance.new("TextBox", mainFrame)
fovInput.Size = UDim2.new(0, 60, 0, 24)
fovInput.Position = UDim2.new(0, 195, 0, 108)
fovInput.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
...
pcall(function() local c = Instance.new("UICorner", fovInput); c.CornerRadius = UDim.new(0, 4) end)
...
fovSlider.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        updateFOV(math.floor(50 + relativeX / sliderWidth * 450))
    end
end)
...
fovInput.FocusLost:Connect(function()
    ...
    if num then updateFOV(num) else fovInput.Text = tostring(FOV_RADIUS) end
end)
...
UserInputService.InputBegan:Connect(function(input, _)
    ...
    if input.KeyCode == Enum.KeyCode.LeftAlt then _AIM = true end
    if input.KeyCode == Enum.KeyCode.C then FLY_KEY_HELD = true end
end)
UserInputService.InputEnded:Connect(function(input, _)
    if input.KeyCode == Enum.KeyCode.LeftAlt then _AIM = false end
    if input.KeyCode == Enum.KeyCode.C then FLY_KEY_HELD = false end
end)
...
local function isAlly(p)
    return not isEnemy(p) and p ~= LocalPlayer
end
...
local function removeAllDrawings(player)
    if pDrawings[player] then for _, d in pairs(pDrawings[player]) do pcall(function() d:Remove() end) end; pDrawings[player] = nil end
    if skLines[player] then for _, l in ipairs(skLines[player]) do pcall(function() l:Remove() end) end; skLines[player] = nil end
    if skHeads[player] then pcall(function() skHeads[player]:Remove() end); skHeads[player] = nil end
end
...
local function removeSkeleton(player)
    if skLines[player] then for _, l in ipairs(skLines[player]) do l.Visible = false; pcall(function() l:Remove() end) end; skLines[player] = nil end
    if skHeads[player] then skHeads[player].Visible = false; pcall(function() skHeads[player]:Remove() end); skHeads[player] = nil end
end
...
local function updateDrawingESP()
    if not DrawingAvailable or not ESP_ENABLED then
        for _, p in ipairs(Players:GetPlayers()) do hideAllDrawings(p); removeSkeleton(p) end
        return
    end
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        local ally = isAlly(p)
        if not SHOW_ALLIES and ally then hideAllDrawings(p); removeSkeleton(p); continue end
        local char = p.Character
        if not char then hideAllDrawings(p); removeSkeleton(p); continue end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        local head = char:FindFirstChild("Head")
        if not hrp or not head then hideAllDrawings(p); removeSkeleton(p); continue end
        local hp, onScreen = Camera:WorldToViewportPoint(head.Position)
        if not onScreen then hideAllDrawings(p); removeSkeleton(p); continue end

        local boxColor, nameColor, skelColor, hpColor = Color3.new(1,0,0), Color3.new(1,1,1), Color3.new(1,1,1), Color3.new(0,1,0)
        if ally then
            boxColor = Color3.new(0, 0.6, 1); nameColor = Color3.new(0.6, 0.8, 1); skelColor = Color3.new(0.6, 0.8, 1); hpColor = Color3.new(0, 0.6, 1)
        end
        if not pDrawings[p] then
            pDrawings[p] = {
                tracer = newDrawing("Line", {Color=boxColor, Thickness=1}),
                box = newDrawing("Square", {Color=boxColor, Thickness=2, Filled=false}),
                nameTag = newDrawing("Text", {Color=nameColor, Size=13, Center=true, Outline=true}),
                hpBg = newDrawing("Square", {Color=Color3.new(0.2,0.2,0.2), Thickness=1, Filled=true}),
                hpBar = newDrawing("Square", {Color=hpColor, Thickness=1, Filled=true})
            }
        else
            pDrawings[p].tracer.Color = boxColor; pDrawings[p].box.Color = boxColor
            pDrawings[p].nameTag.Color = nameColor; pDrawings[p].hpBar.Color = hpColor
        end
        ...
        dw.tracer.From = Vector2.new(sz.X/2, sz.Y); dw.tracer.To = Vector2.new(rootVP.X, rootVP.Y); dw.tracer.Visible = true
        local boxH = math.abs(hp.Y - footVP.Y); local boxW = boxH * 0.5
        dw.box.Size = Vector2.new(boxW, boxH); dw.box.Position = Vector2.new(rootVP.X - boxW/2, hp.Y); dw.box.Visible = true
        dw.nameTag.Text = p.Name.." ["..math.floor(dist).."m]"
        dw.nameTag.Position = Vector2.new(rootVP.X, hp.Y - 15); dw.nameTag.Visible = true
        local health, maxHealth = getHealth(p)
        if health and maxHealth then
            local barWidth = boxW; local barHeight = 3
            ...
            dw.hpBar.Position = dw.hpBg.Position; dw.hpBar.Visible = true
        else
            dw.hpBg.Visible = false; dw.hpBar.Visible = false
        end
        ...
        if SKELETON_ENABLED then
            ...
            if not skLines[p] then skLines[p] = {}; for i=1,14 do skLines[p][i] = newDrawing("Line", {Color=skelColor, Thickness=1}) end
            else for _, l in ipairs(skLines[p]) do l.Color = skelColor end end
            if not skHeads[p] then skHeads[p] = newDrawing("Circle", {Color=skelColor, Thickness=1.5, Filled=false, Radius=SKELETON_HEAD_RADIUS})
            else skHeads[p].Color = skelColor end
            local connections = {
            ...
            for i,conn in ipairs(connections) do
                ...
                if a and b then
                    ...
                    if sa and sb then line.From = Vector2.new(pa.X, pa.Y); line.To = Vector2.new(pb.X, pb.Y); line.Visible = true
                    else line.Visible = false end
                else line.Visible = false end
            end
            if parts["Head"] then
                local hp2, onS = Camera:WorldToViewportPoint(parts["Head"].Position)
                if onS then skHeads[p].Position = Vector2.new(hp2.X, hp2.Y); skHeads[p].Visible = true
...
local function createFallbackESP(player)
    if not isEnemy(player) and not SHOW_ALLIES then return end
    local function setup()
        ...
        bb.Name = "FallbackESP"; bb.Adornee = head; bb.Size = UDim2.new(0,200,0,50); bb.StudsOffset = Vector3.new(0,2.5,0)
        bb.AlwaysOnTop = true; bb.Enabled = false; bb.Parent = head
        local label = Instance.new("TextLabel", bb)
        label.Size = UDim2.new(1,0,1,0); label.BackgroundTransparency = 1; label.TextStrokeTransparency = 0
        ...
        conn = RunService.RenderStepped:Connect(function()
            if not ESP_ENABLED or not player.Character or not player.Character:FindFirstChild("Head") then
                ...
                if bb and bb.Parent then bb:Destroy() end; billboards[player] = nil; return
            end
            if not isEnemy(player) and not SHOW_ALLIES then bb.Enabled = false; return end
            local ally = isAlly(player)
            label.TextColor3 = ally and Color3.new(0,0.6,1) or Color3.new(1,0,0)
            local hrp = player.Character:FindFirstChild("HumanoidRootPart")
            if hrp and Camera then
                local dist = math.floor((Camera.CFrame.Position - hrp.Position).Magnitude)
                local healthText = ""; local health, maxHealth = getHealth(player)
                if health and maxHealth then healthText = " [" .. math.floor(health / maxHealth * 100) .. "%]" end
...
if not DrawingAvailable then
    for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer then createFallbackESP(p) end end
    Players.PlayerAdded:Connect(function(p) if p ~= LocalPlayer then createFallbackESP(p) end end)
end
...
local function closestInFOV()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        if AIM_PART == "Head" then targetPart = char:FindFirstChild("Head")
        else targetPart = char:FindFirstChild("UpperTorso") or char:FindFirstChild("HumanoidRootPart") end
...
local function aimAt(p)
    ...
    if AIM_SMOOTHNESS>=1 then Camera.CFrame=cf else Camera.CFrame = Camera.CFrame:Lerp(cf, AIM_SMOOTHNESS) end
end
...
local function triggerbot()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        if (Vector2.new(hp.X,hp.Y)-sc).Magnitude < FOV_RADIUS and visible(p) then target = p; break end
    end
    if not target then
        if mousePressed then VIM:SendMouseButtonEvent(0,0,0,false,game,false); mousePressed = false end
        return
    ...
    mousePressed = true; lastShot = tick()
    task.delay(0.05, function() if mousePressed then VIM:SendMouseButtonEvent(0,0,0,false,game,false); mousePressed = false end end)
end
...
local function updateFly()
    ...
    if not char or not char:FindFirstChild("HumanoidRootPart") then
        if flyBody then flyBody:Destroy(); flyBody = nil end; return
    end
    local humanoid = char:FindFirstChildOfClass("Humanoid"); local hrp = char.HumanoidRootPart
    ...
    if active then
        ...
        if not flyBody or flyBody.Parent ~= hrp then
            ...
            flyBody.MaxForce = Vector3.new(40000,40000,40000); flyBody.P = 1000
        end
...
print("Финальный скрипт готов: меню с вкладками, подсказка Aimbot: Alt.")-- ESP + Skeleton + Aimbot (Head/Torso) + Triggerbot (на Alt) + Fly (C) + NoClip + Меню с ползунком FOV (аметистовый фон)
